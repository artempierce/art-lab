...
| `backend/artlab/tools/` | Registry (who may call what), tool gateway with per-tool timeout and retry, untrusted wrapper, stub tools, catalog; `save_ideas` (mutating tool, needs approval); `ideas.py` data writer |
| `backend/artlab/tools/approval.py` | Approval gate: pauses graph on mutating tools, sends card to UI, resumes with user's click |
...
| `backend/tests/` | 180 tests on the fake model and local embeddings — no API calls, $0 |
...
7b. **For mutating tools** (e.g. `save_ideas`): the gateway pauses the graph with an **approval interrupt**, sends an `approval` SSE event to the browser showing what the tool will do, and waits for your Approve or Reject click before resuming.
...
| 7–13 | Memory, skills, handoffs, caps, evals, dashboards | planned |
